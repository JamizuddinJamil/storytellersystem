...
import { BriefcaseBusiness, CheckCircle2, Save } from '@lucide/vue'
...
<template>
  <div class="jobs-view">
    <div class="welcome-row"><div><p class="eyebrow">Production operations</p><h2>Job setup</h2><p class="muted">Release approved jobs with the right team and briefing.</p></div></div>
    <p v-if="error" class="form-message form-message--error">{{ error }}</p><p v-if="message" class="form-message">{{ message }}</p>
    <label v-if="selected" class="gdrive-field"><span>Google Drive folder URL</span><input v-model="gdriveUrl" :disabled="!canEdit" type="url" placeholder="https://drive.google.com/..." /></label>
    
    <section class="job-layout">
      <div class="panel job-list"><div class="panel-heading"><h3>Jobs</h3><BriefcaseBusiness :size="20" class="panel-icon" /></div><div v-for="job in jobs" :key="job.id" class="job-row" :class="{ 'job-row--active': selected?.id === job.id }"><button type="button" @click="selectJob(job)"><span><strong>{{ job.job_number }}</strong><small>{{ job.event_type }} · {{ job.event_date }}</small></span><span class="status-badge">{{ job.status }}</span></button><button v-if="job.status !== 'CANCELLED' && job.status !== 'DELIVERED'" class="icon-button icon-button--danger" type="button" aria-label="Cancel job" @click="cancelJob(job)">×</button></div><span v-if="!jobs.length" class="pipeline-empty">No jobs ready for setup</span></div>
      <form v-if="selected" class="panel job-editor" @submit.prevent="saveSetup">
      ...
      <div v-else class="panel empty-state"><BriefcaseBusiness :size="28" /><strong>Select a job</strong><span>Choose a job to configure its production team.</span></div>
      <div v-if="selected" class="panel assignment-list"><h3>Assignment requests</h3><div v-for="assignment in assignments" :key="assignment.id" class="assignment-row"><span><strong>{{ assignment.profile?.full_name || assignment.role }}</strong><small>{{ assignment.role }} · {{ assignment.status }}</small></span><span v-if="assignment.status === 'CLAIMED'" class="client-actions"><button class="text-button" type="button" @click="confirmAssignment(assignment, true)">Confirm</button><button class="text-button danger-button" type="button" @click="confirmAssignment(assignment, false)">Decline</button></span></div><span v-if="!assignments.length" class="pipeline-empty">No assignment requests</span></div>
    </section>
  </div>
</template>